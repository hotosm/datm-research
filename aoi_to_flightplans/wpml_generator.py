...
def calculate_gsd(
    ...
    gsd = (sensor_width * flight_altitude * 100) / (
        focal_length * image_width
    )
    return gsd  # (cm/pixel)

def calculate_image_interval(
    gsd: float,
    flight_speed: int,
    overlap: int,
    image_height,
) -> float:
    """Calculate image interval from gsd, flight speed, altitude, overlap."""
    # Vertical image footprint in cm
    vertical_image_footprint = (gsd * image_height) / 100

    # Distance the drone covers between images
    image_interval_distance = (vertical_image_footprint / (100 / (100 - overlap)))

    # Image interval based on the flight speed
    image_interval = image_interval_distance / flight_speed

    return image_interval

def calculate_wayline_spacing(
    gsd: float, overlap: int, image_width: int,
):
    overlap_fraction = overlap / 100
    image_width_in_cm = gsd * image_width
...
def generate_waypoints_within_polygon(
    ...
    if "features" in aoi:
        ...
        polygon = Polygon(aoi["geometry"]["coordinates"][0])
...
def create_zip_file(waylines_path_uid, filename):
    ...
    output_file_name = f"/tmp/{waylines_path_uid}/{filename}.zip"
...
def create_xml(placemarks, finish_action, generate_each_points: bool = False, filename="output"):
    ...
    output_file_name = create_zip_file(folder_name, filename)
    return output_file_name
